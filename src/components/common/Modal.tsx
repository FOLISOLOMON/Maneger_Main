      ...
      >
        {title && (
          <div className="flex items-start justify-between gap-3 px-5 pt-5 pb-3 border-b border-slate-100">
            ...
            <button
              onClick={onClose}
              className="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors touch-target"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        {footer && (
          <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl flex gap-2 justify-end">
            {footer}
          </div>
        )}
      </div>
